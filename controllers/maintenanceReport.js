...
export const createMaintenanceReport = asyncErrorHandler(
  async (req, res, next) => {
    ...
    try {
      await Device.findOneAndUpdate(
        { name: maintenanceReportInput.device_name },
        { $set: { status: maintenanceReportInput.working_status } }
      );
...
export const getListMaintenanceReport = asyncErrorHandler(
  async (req, res, next) => {
    try {
...
export const sortReportByDevice = asyncErrorHandler(async (req, res, next) => {
  try {
    MaintenanceReport.find({ device_name: req.body.deviceName }).then(
      (report) => {
        res.send(report);
      }
    );
...
export const getSingleMaintenanceReport = async (req, res) => {
  try {
    MaintenanceReport.findById({ _id: req.params.id }).then(
      (maintenanceReport) => {
...
export const updateMaintenanceReport = asyncErrorHandler(
  async (req, res, next) => {
    try {
      await MaintenanceReport.findOneAndUpdate(
        { _id: req.params.id },
        {
          $set: req.body,
        }
